...
function LandingPage() {
    ...
    useEffect(() => {
        ...
        const downArrow = document.querySelector('.down-arrow-container');
        const feedPoint = document.querySelector('.feed.point').getBoundingClientRect().top - 200;
        const messagePoint = document.querySelector('.message.point').getBoundingClientRect().top + 400;
        const profilePoint = document.querySelector('.profile.point').getBoundingClientRect().top + 1000;
        ...
        const handleScroll = () => {
            ...
            if (window.scrollY >= feedPoint) {
                appear(sampleFeed);
            } 
            if (window.scrollY >= messagePoint) {
                appear(sampleMessages);
            } 
            if (window.scrollY >= profilePoint) {
